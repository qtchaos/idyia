...
const existingAdmin = await db.select({ id: schema.user.id })
	.from(schema.user).where(eq(schema.user.email, 'admin@idyia.local')).get();
...
if (!existingAdmin) {
	...
	await db.insert(schema.user).values({
		id: adminId, name: 'Admin', email: 'admin@idyia.local',
		emailVerified: true, createdAt: new Date(), updatedAt: new Date(),
	});
	await db.insert(schema.account).values({
		id: crypto.randomUUID(), accountId: adminId, providerId: 'credential',
		userId: adminId, password, createdAt: new Date(), updatedAt: new Date(),
	});
	await db.insert(schema.userRole).values({ userId: adminId, role: 'admin' })
...
const prefixes  = ['Nord','Tele','Omni','Neo','Meta','Hyper','Ultra','Super','Cyber','Data',
                   'Auto','Eco','Bio','Geo','Aero','Nano','Macro','Micro','Smart','Fast'];
const middles   = ['flux','ware','core','base','link','sync','flow','grid','hub','lab',
                   'net','tech','sys','soft','logic','mind','wise','forge','works','space'];
const suffixes  = ['','','','', 'AI', 'AI', 'Labs', 'Group', 'Co', 'Systems', 'Technologies',
                   'Digital', 'Solutions', 'Studio', 'HQ'];

const tlds      = ['com','io','ai','ee','eu','co','tech','app'];

const types: schema.CompanySizeCode[] = ['A','B','C','D','E','F','G','H','I'];

const companyTypes = ['restaurant','saas','government','institution','retail',
                      'finance','healthcare','media','education','other'] as const;
...
const sourceDomains = [
	'twitter.com','x.com','linkedin.com','techcrunch.com','wired.com',
	'venturebeat.com','web.archive.org','reuters.com','bloomberg.com','theverge.com',
	'medium.com','substack.com','blog.','news.','press.',
];
...
function makeRegistered(name: string, tld: string): { registeredName: string | null; registryUrl: string | null } {
	...
	const baseName      = name.replace(/ (Labs|Group|Co|Systems|Technologies|Digital|Solutions|Studio|HQ|AI)$/, '');
	return {
		registeredName: `${baseName} ${legalSuffix}`,
		registryUrl:    Math.random() < 0.6 ? `https://registry.example.com/${baseName.toLowerCase()}` : null,
	};
}
...
for (let i = 0; i < count; i++) {
	let name: string;
	do { name = makeName(); } while (generated.has(name));
	generated.add(name);
	...
	companies.push({
		name, registeredName, registryUrl, website,
		companyType: type,
		companySize: size,
		description,
...
for (let i = 0; i < companies.length; i += batchSize) {
	await db.insert(schema.companies).values(companies.slice(i, i + batchSize)).onConflictDoNothing();
}

console.log(`✓  inserted ${companies.length} companies (${companies.filter(c => c.status === 'approved').length} approved, ${companies.filter(c => c.status === 'pending').length} pending)`);
process.exit(0);
